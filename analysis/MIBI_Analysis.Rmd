...
neighbor.dists %>%
  ...
  labs(subtitle = "Distribution of the mean distances to the 10, 20, 40, or 80 closest neighbours.")
```
...
Let's look at the correlation of those metrics with the mixing score. The
correlation of the sum of the gain.R2 is driven by only 3 samples, so I am not
quite sure whether one could consider this important.

```{r fig.height=4, fig.width=10}
ranger.results$improvements %>%
  filter(measure == "gain.R2") %>% 
  filter(target %in% c("CD138", "CD209", "phospho.S6", "IDO", "EGFR")) %>%
  mutate(sample = str_extract(sample, "(?<=_s)[0-9]+")) %>%
  group_by(sample) %>%
  ...
  geom_smooth(aes(x = ratio.tumor.immune, y = value),
              formula = y ~ x, method = "lm") +
  facet_wrap(~ stat, ncol=3)
```

What else could it be correlated with?
...
plots <- map(c("Count.Immune", "Count.Tumor", "Count.Endothelial", "Count.Mesenchymal.like"),
              function(test.var) {
  ranger.results$improvements %>%
    filter(measure == "gain.R2") %>% 
    filter(target %in% c("CD138", "CD209", "phospho.S6", "IDO", "EGFR")) %>%
...
Looking at the correlation as is. We can basically see that the trend is
rather driven by 3 samples.

```{r fig.height=4, fig.width=8}
p1 <- ranger.results$improvements %>%
  filter(measure == "gain.R2") %>% 
  filter(target %in% c("CD138", "CD209", "phospho.S6", "IDO", "EGFR")) %>%
  mutate(sample = str_extract(sample, "(?<=_s)[0-9]+")) %>%
  group_by(sample) %>%
  summarise(sum.gain.R2 = sum(value), .groups = "drop") %>%
  left_join(sample.meta, by = c("sample" = "SampleID")) %>%
  ggplot() +
  geom_point(aes(x = ratio.tumor.immune, y = sum.gain.R2)) +
  geom_smooth(aes(x = ratio.tumor.immune, y = sum.gain.R2),
              formula = y ~ x, method = "lm")

p2 <- ranger.results$improvements %>%
  filter(measure == "gain.R2") %>% 
  filter(target %in% c("CD138", "CD209", "phospho.S6", "IDO", "EGFR")) %>%
  mutate(sample = str_extract(sample, "(?<=_s)[0-9]+")) %>%
  group_by(sample) %>%
