...
raw_data %>% slice_head(n=6) %>% knitr::kable(digits = 2)
```
...
meta <- data %>%
  select(c(1:3, 53:59))
meta %>% slice_head(n=6) %>% knitr::kable(digits = 2)

expr <- data %>%
  select(4:52)
expr %>% slice_head(n=6) %>% knitr::kable(digits = 2)
```
...
patient.info.paper %>% slice_head(n=6) %>% knitr::kable(digits = 2)
```
...
which cutoff value should be used for the generation of the juxtaview.
...
10 clostest, 20 closest, 40 clostest, or 80 clostest cells to get a feeling
...
How is the gain.R2 dependend on the sample and the target. How could I 
decompose those effects? 
...
Let's classify the samples based on the hierachical clustering of the 
"gain.R2" signature as shown in the heatmap.
...
Now we may like to investigate the proportion of the classification in more
detail across the clusters.
...
Now could it be that the the intraview R2 could somehow explain the clustering
