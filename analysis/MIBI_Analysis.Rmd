...
```{r fig.height=14, fig.width=20}
plot_diagnostics(earth.results.smp$s1, name="s1", low = .5)
```
...
all.measures <- rbind(
  ranger.measures %>% mutate(method = "ranger"),
  earth.measures %>% mutate(method = "earth")
)
```
...
plot_comparison <- function(df, m) {
  ...
  theme(axis.text.x.bottom = element_text(vjust = 0.4))
}
```
...
So overall the see the same trend for earth (MARS) and ranger (RF), but
the performance of ranger is better in almost all cases. The gain in 
variance explained and overall R2 of the multiview are consistently 
higher.

```{r fig.width=10, fig.height=6}
plot_comparison(all.measures, "gain.R2")
```

### Multi R2

```{r fig.width=10, fig.height=6}
plot_comparison(all.measures, "multi.R2")
```

### Intra R2

```{r fig.width=10, fig.height=6}
plot_comparison(all.measures, "intra.R2")
```
...
```{r}
all.contributions <- map2_dfr(results.smp, names(results.smp), 
                              function(smp, name) {
  smp$contributions %>%
    select(target, view, value) %>%
    filter(!str_starts(view, "p\\."), view != "intercept") %>%
    mutate(sample = name)
  })

sum.contributions <- all.contributions %>%
  group_by(target, view) %>%
  summarise(sum = sum(value)) %>% 
  # remove negative values here?
  mutate(sum = ifelse(sum < 0, 0, sum)) %>%
  pivot_wider(names_from = view, values_from = sum) %>%
  # harcoded names here!
  mutate(total = sum(intra, juxta.40, para.120)) %>%
  mutate(rel.intra = intra/total,
         rel.juxta = juxta.40/total,
         rel.para = para.120/total) %>%
  select(c(starts_with("rel"), target)) %>%
  pivot_longer(cols = starts_with("rel"))

ggplot(sum.contributions, aes(x = target, y = value)) +
  geom_col(aes(group = name, fill = name)) +
  scale_fill_brewer(palette = "Set2") +
  theme_classic() +
  labs(x = "Target", y = "Contribution") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```

Visualizing the contriubtions using boxplots for each target, we can 
see again the the intra and the paraview are most important, whereas
the juxtaview is rather unimportant.

```{r fig.width=6, fig.height=8}
all.contributions %>%
  ggplot() +
  geom_boxplot(aes(x=view, y=value), 
               col = rep(c("forestgreen", "tomato", "slateblue"), 36)) +
  facet_wrap(~ target) +
  labs(x = "Target", y = "Contribution") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
How should we aggregate the importances?

```{r}
all.importances <- map2_dfr(results.smp, names(results.smp), 
                              function(smp, name) {
  smp$importances %>%
    mutate(sample = name)
  })
...
For example summing up the importances from all samples like that:

```{r fig.height=6, fig.width=14}
all.importances %>%
  group_by(view, Predictor, Target) %>%
  summarise(Sum.Imp = sum(Importance)) %>%
  ggplot(aes(x = Predictor, y = Target)) +
    geom_tile(aes(fill = Sum.Imp)) +
...
Or rather taking the mean like that?

```{r fig.height=6, fig.width=14}
all.importances %>%
  group_by(view, Predictor, Target) %>%
  summarise(Mean.Imp = mean(Importance)) %>%
  ggplot(aes(x = Predictor, y = Target)) +
    geom_tile(aes(fill = Mean.Imp)) +
    facet_wrap(~ view, ncol = 3) +
    theme_classic() +
    theme(axis.text.x = ggplot2::element_text(angle = 90, hjust = 1)) +
    coord_equal() +
...
sample.meta <- patient.info %>%
  filter(SampleID != 30) %>%
  mutate(ratio.tumor.immune = mixing.score) %>%
