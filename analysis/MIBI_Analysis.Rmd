...
The observed hierachical clustering is basically a signature of the samples.
...
Let's cluster the samples based on the "gain.R2 signature" and see what is
...
Now visualize the heatmap again with the chosen cluster "depth". However, first
we will remove all clusters with only one sample.
...
From another perspective we could bascially only look at the top targets and summarize things.
