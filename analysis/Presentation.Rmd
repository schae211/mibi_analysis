...
intracellular information (intraview) and then information from the spatial 
context (multiview). 

- If the R2 values for the multiview (gain.R2) is higher, this shows that taking 
the spatial context (epxression of all other markers in the neighboring cells) 
has predictive power for the target.

- Thus, we will first look at the gain in explained variance accross all targets
...
to the plot the overall differences between compartimentalized and
mixed tumors are not to big.
...
compartimentalized and mixed samples? Meaning for the prediction 
of which targets is it important whether the special context comprises
a structured or unstructed TME?

- On top we can see the targets for which the gain in R2 when taking the spatial
context into consideration is significantly different between 
...
ranger.results$improvements %>%
  ...
  scale_color_manual(values = c("compartimentalized" = "forestgreen", "mixed" = "blue")) +
...
- Let's contrast the importances of the paraview for compartimentalized and mixed samples
...
- What kind of interactions are important here?
...
notumor.misty.results$improvements %>%
  ...
  scale_color_manual(values = c("compartimentalized" = "forestgreen", 
                                "mixed" = "blue")) 
```
