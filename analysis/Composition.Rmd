...
```{r}
to.remove <- c("Class_Tregs", "Class_DC", "Class_NK", "Class_Other.immune")
misty.views.red <- map(misty.views.smp, function(misty.view) {
  if ("Class_DC" %in% colnames(misty.view$intraview$data)) {
    misty.view$intraview$data <- misty.view$intraview$data %>%
      mutate(Class_DC.Mono = Class_DC.Mono + Class_DC)
