...
However, the mean p-values are rather hight because there is such a large
...
cancer.immune.results$improvements %>%
  ...
  summarise(mean.measure = mean(value, na.rm=TRUE)) %>%
...
cancer.immune.results$improvements %>%
  ...
  filter(target %in% c("IDO", "PD.L1", "PD1")) %>%
  group_by(target, Classification, measure) %>%
  summarise(mean.measure = median(value, na.rm=TRUE)) %>%
  pivot_wider(names_from = measure, values_from = mean.measure)
```
...
This is not the case for PD-L1.
...
Now in general how is the mixing score correlated with the gain in variance
for all targets.
...
cancer.immune.results$improvements %>%
  ...
  filter(ratio.tumor.immune < 300) %>%
  group_by(target) %>%
  summarise(c = cor(value, ratio.tumor.immune)) %>%
  ggplot() +
  geom_bar(aes(x=reorder(target, -c), y=c), stat="identity") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
